#pragma compile(Out,和越报关数据自动录入(金二版)2023.exe)
#pragma compile(FileDescription, 和越报关数据处理系统 - 数据自动识别及录入)
#pragma compile(ProductName, 和越报关数据自动录入(2022分原产地版本))
#pragma compile(Icon, InputData.ico)
#pragma compile(FileVersion, 3.18.07.10, 3.18.99.99)
#include-once
...
#include "MakeSingleWindowXML_V5-HY.au3"
#include "SqlitFunc.au3"

Local $version="3.23.04.20"
;检查授权
...
Func Start()
    ...
    Local $hGUI = GUICreate("和越数据整理录入",620,600)

    ; Set Margins
;~     _GUICtrlEdit_SetMargins($g_idMemo, BitOR($EC_LEFTMARGIN, $EC_RIGHTMARGIN), 20, 20)

    Global $idBtnSelectBooking = GUICtrlCreateButton("Booking", 50, 50, 55, 30)
    Global $idBtnBookDirZip = GUICtrlCreateButton("ZIP", 105, 50, 24, 30)
    Global $g_idBookingData = GUICtrlCreateEdit("", 150, 50, 400, 40)
    Global $idBtnClearBooking = GUICtrlCreateButton("X", 550, 50, 20, 40)
    Global $idBtnSelectDir = GUICtrlCreateButton("数据准备", 50, 100, 80, 30)
    Global $idBtnSaveOutput = GUICtrlCreateButton("保存", 50, 150, 80, 30)
    Global $idBtnSelectFile = GUICtrlCreateButton("数据文件", 150, 100, 70, 30)
...
;~     Global $idBtnZhuanGuan = GUICtrlCreateButton("转关录入", 450, 150, 100, 30)
    Global $idBtnTEST = GUICtrlCreateButton("收取回执", 150, 150, 80, 30)
    Global $idBtnINVListSave = GUICtrlCreateButton("核注[暂存]", 264, 150, 80, 30)
    Global $idBtnINVListApply = GUICtrlCreateButton("核注[申报]", 378, 150, 80, 30)
    Global $idBtnSingleWindow = GUICtrlCreateButton("报关[暂存]", 492, 150, 80, 30)
;~     Global $idBtnClose = GUICtrlCreateButton("关闭", 480, 150, 70, 30)
    Global $g_idOutputData = GUICtrlCreateEdit("", 230, 100, 320, 40)
    Global $idBtnClearOut = GUICtrlCreateButton("X", 550, 100, 20, 40)
...
;~ 	_GUICtrlEdit_AppendText($g_idMemo,StringFormat("%s  %s" & @CRLF,_Date_Time_SystemTimeToDateTimeStr(_Date_Time_GetLocalTime(),1),"和越数据整理录入"))
	...
	WriteMemo(0,"和越数据整理录入 "&$version&"","",False)
    ; Display the GUI.
...
Func zgCheckInformation($ContractNO)
   ...
   ["",					"",							"TO:"], _					;1.2.10.
   ["CONTRACT NO: ",	"Invoice No.",				"PACKING NO.    :  "], _	;3.
   ["",					"",							"SAY TOTAL "], _			;4.5.
   ["Quantity",			"",							"TOTAL"], _					;6.7.9.
   ["Specifications",	"",							""], _						;8.
   ["Total Value",		"Invoice Total",			""], _						;11.
...
;~ *****************************************************
   ...
   Local $Qty=0,$TotalValue=0,$MakeCountryWet=0
	  $ret = zgCheckMakeCountry($aExcelExpci,$Qty,$TotalValue,$MakeCountryWet,$aSignature[11][1],1,7,$ContractNo,$Country)	;返回原产国商品条数，在参数中带回数量，价值，重量
	  If $ret > 0 Then
		 $aTmp[0][0] = $ContractNO
		 $aTmp[0][1] = $Country
		 $aInput[0][9] = $Country
		 ...
		 ;12-1   12-4
		 $sTmp2 = zgFindExcelArrayMakeCountry($aExcelExpci,$aSignature[6][1],0,0,$Country)
		 $aTemp = StringSplit($sTmp2," ")
		 $aTmp[0][5]=$aTemp[1]
		 $aTmp[0][6]=StringRight($sTmp2,StringLen($sTmp2)-StringLen($aTmp[0][5])-1)

		 ;12-2
		 $sTmp2 = zgFindExcelArrayMakeCountry($aExcelExpci,$aSignature[7][1],0,0,$Country)
